...
export function SettingsView() {
  ...
  const [showAppSyncSetup, setShowAppSyncSetup] = useState(false);
  const [showJoinSync, setShowJoinSync] = useState(false);
  const [showLinkCode, setShowLinkCode] = useState(false);

  // Load provider metadata and app sync status on mount.
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-foreground">App Sync</h2>
            {appSync.status?.configured && !appSync.status.isSyncing && (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => appSync.push()}
          ...
          {appSync.error && (
            <div className="flex items-start gap-2 px-3 py-2 mb-3 rounded-md bg-destructive/10 text-destructive text-xs">
              <WifiOff className="h-4 w-4 flex-shrink-0 mt-0.5" />
              <p className="break-all">{appSync.error}</p>
            </div>
          )}
          ...
          {!appSync.status?.configured ? (
            <div className="border border-dashed border-border rounded-md p-4 text-center space-y-3">
              <div className="flex justify-center">
                <MonitorSmartphone className="h-7 w-7 text-muted-foreground/40" />
              </div>
      ...
      {showAppSyncSetup && (
        <AccountModal title="Set Up App Sync" onClose={() => setShowAppSyncSetup(false)}>
          <AppSyncSetupForm
            onDone={() => { setShowAppSyncSetup(false); appSync.loadStatus(); }}
          />
        </AccountModal>
      )}
      ...
      {/* App Sync join modal */}
      {showJoinSync && (
        <AccountModal title="Join with Link Code" onClose={() => setShowJoinSync(false)}>
          <AppSyncJoinForm
            onDone={() => { setShowJoinSync(false); appSync.loadStatus(); }}
          />
        </AccountModal>
      )}

      {/* App Sync link code modal */}
      {showLinkCode && (
        <AccountModal title="Link Code" onClose={() => setShowLinkCode(false)}>
          <AppSyncLinkCodePanel onClose={() => setShowLinkCode(false)} />
        </AccountModal>
      )}
    </div>
  );
...
function AppSyncSetupForm({ onDone }: { onDone: () => void }) {
  const appSync = useAppSyncStore();
  ...
  async function handleTest() {
    ...
    try {
      ...
      // testConnection throws on failure (Rust returns Err with a real message),
      // or sets appSync.error. Either way, if we get here it succeeded.
    } catch (e) {
      setError(String(e));
    } finally {
      // Also surface any error the store captured (e.g. from a non-throwing path).
...
function AppSyncLinkCodePanel({ onClose }: { onClose: () => void }) {
  const appSync = useAppSyncStore();
